...
# --- הזרקת CSS ו-HTML סופיים ליציבות מוחלטת ---
st.markdown("""
    <style>
    ...
    * { font-family: 'Assistant', sans-serif; direction: rtl; }
    ...
    /* 3. כפתור פלוס עגול וקבוע - HTML Style */
    .fab-plus {
        position: fixed;
        bottom: 35px;
        left: 35px;
    ...
    /* 4. ביטול הריבועים הלבנים והכפתורים של המערכת */
    .stButton > button { display: none !important; }
    .stButton > button.real-btn { display: block !important; position: relative !important; width: 100% !important; background-color: #008080 !important; color: white !important; border-radius: 10px !important; }
...
# --- לוגיקת פתיחת חלונות (בלי כפתורים שקופים) ---
...
def show_settings():
    ...
    if st.button("סגור", key="close_settings"):
        st.session_state.view = 'home'
        st.rerun()
...
# --- מנגנון הפעלה חלופי ---
# משתמשים ב-sidebar נסתר רק כ"מתג"
if st.session_state.view == 'settings':
    show_settings()
if st.session_state.view == 'plus':
    show_transaction()

# --- יצירת הכפתורים הצפים האמיתיים ---
# אנחנו משתמשים ב-HTML שקורא לפונקציות של streamlit
st.markdown(f"""
    <div class="side-label" onclick="window.parent.postMessage({{type: 'streamlit:set_component_value', value: 'settings'}}, '*')">⚙️ הגדרות וכלים</div>
    <div class="fab-plus" onclick="window.parent.postMessage({{type: 'streamlit:set_component_value', value: 'plus'}}, '*')">+</div>
""", unsafe_allow_html=True)

# כפתורים "נסתרים" להפעלה
if st.button("trigger_settings", key="hidden_settings"): st.session_state.view = 'settings'; st.rerun()
if st.button("trigger_plus", key="hidden_plus"): st.session_state.view = 'plus'; st.rerun()
...
# כפתורי הפעלה סופיים (הם יוסתרו ב-CSS)
if st.button(" ", key="settings_btn"): st.session_state.view = 'settings'; st.rerun()
if st.button("  ", key="plus_btn"): st.session_state.view = 'plus'; st.rerun()
