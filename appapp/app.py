...
# 2. הזרקת CSS - רק לעיצוב הכפתורים ושמירה על העיצוב הקיים
st.markdown("""
    <style>
    ...
    * { font-family: 'Assistant', sans-serif; direction: rtl; }
    ...
    /* עיצוב כפתור הפלוס - יציב בפינה השמאלית */
    ...
    /* עיצוב תווית הגדרות - יציב בפינה הימנית */
    ...
    /* שמירה על כרטיסיות הנתונים המקוריות */
...
# --- פונקציות חלונות מרחפים (Dialogs) ---
...
def show_transaction():
    ...
    st.text_area("פירוט (Description):", placeholder="הוסיפי פירוט כאן...")
    if st.button("אישור ושמירה", use_container_width=True):
        st.balloons(); st.rerun()

# --- תוכן דף הבית (העיצוב המקורי) ---
...
# --- הכפתורים החסרים (מוצבים בסוף כדי שלא יופיעו ריבועים למעלה) ---
